...
| Changes | `changes` | **check-outdated** tries to find a direct link to changelog of the package. The following places are considered in the given order:<br />1. node_modules/{name}/CHANGELOG<span>.</span>md<br />2. node_modules/{name}/package.json > "repository" \*<br />3. node_modules/{name}/package.json > "homepage"<br />4. `https://www.npmjs.com/package/{name}`<br />\* GitHub-repository URLs are adjusted, so that they directly link to the **Releases** section. | https<span>:</span>//github<span>.</span>com/Microsoft/TypeScript/releases |
| Homepage | `homepage` | An URL with additional information to the package. The following places are considered in the given order:<br />1. node_modules/{name}/package.json > "homepage"<br />2. node_modules/{name}/package.json > "repository"<br />3. node_modules/{name}/package.json > "author"<br />4. `https://www.npmjs.com/package/{name}` | https<span>:</span>//www<span>.</span>typescriptlang<span>.</span>org/ |
